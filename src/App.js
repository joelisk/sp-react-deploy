...
import CustomRange from './Slider'; 
import './App.css';
        ...
        <div>
          <header className = "header">
              <div className="container">
                ...
                onStartYearChange={this.handleStartYearChange}
                onEndYearChange={this.handleEndYearChange}
                   />
                   <span className="range-text">1926-2019</span>
...
class Table extends React.Component {
  ...
  render() {
      ...
      data.map((datum)=>{
        const totalReturn = parseFloat(datum.totalReturn,2)
          if(datum.year >= startYear && datum.year <= endYear) { 
              ...
              console.log(cumulativeArray.length)

              function cumulativeAdd(){
                let cumulative = 0 
              for (let i = 0; i < cumulativeArray.length; i++){
                cumulative+= cumulativeArray[i]
              }
               return cumulative
              }
              ...
              rows.push(
                  <TableRow
                      year = {datum.year}
                      totalReturn = {datum.totalReturn}
                      cumulative = {cumulativeAdd()}
                      key={datum.year}/>,
      ...
      return(
          <div>
             <span className="title">S & P 500 Returns </span>
              <table className="table">
                  <thead>
                  <span className="table-text">S & P Total Returns by Year</span>
...
class TableRow extends React.Component{
  render() {
      let n = this.props.cumulative
      let nTwo = n.toFixed(2)
      return(
          <tr style = {{borderBottom: 'grey'}} className="table-row">
              <td>{this.props.year}</td>
              <td className={`${parseInt(this.props.totalReturn)<0?"red":""}`}>{this.props.totalReturn}</td>
              <td>{nTwo}</td>
          </tr>
      );
  }
